...
export default class SceneStart extends Scene {
    ...
    processInput(keyListener) {
        [...document.querySelector('.gender').children].forEach((element) => element.onclick = () => {
    ...
    update(elapsed) {
        if (this.starting)
            return new SceneInstructions(this.maxX, this.maxY, new Level1(this.maxX, this.maxY, this.initiatePlayer = new Player(this.maxX / 2, 550), 0, null), 'People with [!] above their heads are important.', 'People with [?] above their heads have something useful.', 'Press [E] to interact.', '[W], [A], [S], and [D] to move around the map,');
        return null;
    }
    render(canvas) {
